...
public class TranslationRequestForm extends DefaultFormHandler {
	private static Log log = LogFactory.getLog(TranslationRequestForm.class);
	...
	public void adjustFormInstance(FormInstanceCreationContext context, FormInstance formInstance)
		...
		addFormLabelParameter(params, "baseMo",
				"Request translations for \"" + ts.getBaseMo().getDisplayName() + "\" (" + ts.getBaseLang() + "):");
		int trans = 0;
		for (ManagedObject mo : ts.getTranslationsMos()) {
			...
			if (mo.getLayeredMetadataValue(TranslationConstants.LMD_TRANSLATION_STATE) == null
					|| !mo.getLayeredMetadataValue(TranslationConstants.LMD_TRANSLATION_STATE).equals(TranslationConstants.STATUS_TRANSLATION_IN_PROGRESS_VALUE)) {
				List<DataTypeOptionValue> options = new ArrayList<DataTypeOptionValue>();
				options.add(new DataTypeOptionValue(lang,
						lang + "/" + langMap.get(lang) + " " + TranslationSetUtils.getTranslationStateHtml(context, user, mo)));
				addFormSelectTypeParameter(FormControlType.CHECKBOX, params, "lang", "", nullDataType, options,
						nullValues, sortNoSort, allowMultiple, notRequired, notReadOnly);
				trans++;
			}
		...
		for (String l : alphaKeys) {
			List<DataTypeOptionValue> options = new ArrayList<DataTypeOptionValue>();
			options.add(new DataTypeOptionValue(l, l + "/" + langMap.get(l) + " - Not yet created"));
		...
		addFormLabelParameter(params, "inProgress", "Translations already in progress:");
		int rc = 0;
		for (ManagedObject mo : ts.getTranslationsMos()) {
			if (mo.getLayeredMetadataValue("translationState") != null
					&& mo.getLayeredMetadataValue(TranslationConstants.LMD_TRANSLATION_STATE).equals(TranslationConstants.STATUS_TRANSLATION_IN_PROGRESS_VALUE)) {
				String transDate = mo.getLayeredMetadataValue(TranslationConstants.LMD_TRANSLATION_REQUEST_DATE);
				String lang = mo.getLayeredMetadataValue(TranslationConstants.LMD_LANGUAGE);
				List<DataTypeOptionValue> options = new ArrayList<DataTypeOptionValue>();
				options.add(new DataTypeOptionValue(lang, langMap.get(lang) + " (" + lang + ") "
						+ TranslationSetUtils.getTranslationStateHtml(context, user, mo) + " Requested " + transDate));
				String[] val = new String[1];
				val[0] = lang;
				addFormSelectTypeParameter(FormControlType.CHECKBOX, params, "langInProgress", "", nullDataType,
